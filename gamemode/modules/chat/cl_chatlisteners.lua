...
local function drawChatReceivers()
    ...
    -- No one hears you
    if #receivers == 0 then
        draw.WordBox(2, x, y, DarkRP.getPhrase("hear_noone", currentConfig.text), "FalloutRP2", Color(0,0,0,160), Color(255,0,0,255))
        return
    -- Everyone hears you
    elseif #receivers == #player.GetAll() - 1 then
        draw.WordBox(2, x, y, DarkRP.getPhrase("hear_everyone"), "FalloutRP2", Color(0,0,0,160), Color(0,255,0,255))
        return
    end

    draw.WordBox(2, x, y - (#receivers * 21), DarkRP.getPhrase("hear_certain_persons", currentConfig.text), "FalloutRP2", Color(0,0,0,160), Color(0,255,0,255))
    for i = 1, #receivers, 1 do
        ...
        draw.WordBox(2, x, y - (i - 1) * 21, receivers[i]:getName(), "FalloutRP2", Color(0, 0, 0, 160), Color(255, 255, 255, 255))
    end
end
...
local function loadChatReceivers()
    ...
    DarkRP.addChatReceiver("/pm", "PM", function(ply, text)
        ...
        return string.find(string.lower(ply:getName()), text[2], 1, true) ~= nil or
            string.find(string.lower(ply:SteamID()), text[2], 1, true) ~= nil or
    ...
    DarkRP.addChatReceiver("speak", DarkRP.getPhrase("speak"), function(ply)
        ...
        return not true or ply:isInRoom() // Only people in same room can hear you
    end)
end
