...
/* Not needed, used for setting jailpos, and other commands
...
local function RP_PlayerChat(ply, text, teamonly)
    //DarkRP.log(ply:getName() .. " (" .. ply:SteamID() .. "): " .. text)
    local callback = ""
    local DoSayFunc
    ...
    if string.sub(text, 1, 1) == "/" and tblCmd then
...
function GM:canChatCommand(ply, cmd, ...)
    if ply.loaded then return true end

	ply:notify("Data not loaded yet.", NOTIFY_ERROR)
...
function GM:PlayerSay(ply, text, teamonly) -- We will make the old hooks run AFTER DarkRP's playersay has been run.
    ...
    if game.IsDedicated() then
        ServerLog("\"" .. ply:getName() .. "<" .. ply:UserID() .. ">" .. "<" .. ply:SteamID() .. ">" .. "<" .. team.GetName(ply:Team()) .. ">\" say \"" .. text .. "\"\n" .. "\n")
    end
...
local function ReplaceChatHooks()
    ...
    if oldMetatable ~= nil then
        ErrorNoHalt("Some addon is fucking up DarkRP's chat hook reorganising mechanism. Start getting rid of scripts and addons until you don't see this error on startup anymore.")
        return
    end
