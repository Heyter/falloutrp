
SKILLPOINTS_LEVEL = 5 // How many skill points the player receives when they level up

SKILLS = {
	{Name = "Barter", Description = "Barter reduces the buying price \n from vendors and also increases \nthe selling price to vendors."},
	{Name = "Energy Weapons", Description = "Increases the damage done \nby energy weapons."},
	{Name = "Explosives", Description = "Increases the damage done \nby explosives."},
	{Name = "Guns", Description = "Increases the damage done by guns."},
	{Name = "Lockpick", Description = "Lockpick allows chest and doors\nto be picklocked if they \nrequire a certain level."},
	{Name = "Medicine", Description = "Medicine increases the health \nand health regeneration \nreceived from aid items."},
	{Name = "Melee Weapons", Description = "Increases the damage done \nby melee weapons."},
	{Name = "Repair", Description = "Reduces the chance to destroy \nitems when repairing."},
	{Name = "Science", Description = "Increases plasma weapon damage."},
	{Name = "Sneak", Description = "Under development."},
	{Name = "Speech", Description = "Under development."},
	{Name = "Survival", Description = "Decreases NPC damange and increases\nhunger and thirst quench."},
	{Name = "Unarmed", Description = "Increases damage done by unarmed."}
}

// Barter
PRICE_MULTIPLIER_BARTER = 0.01 // How much buying/selling price is reduced/increased (multiplier) for each point of barter

// Energy Weapons
DAMAGE_MULTIPLIER_ENERGYWEAPONS = 0.005 // How much damage is increased (multiplier) for energy weapons for each point of energy weapons

// Explosives
DAMAGE_MULTIPLIER_EXPLOSIVES = 0.03 // How much damage is increased (multiplier) for explosives for each point of explosives

// Guns
DAMAGE_MULTIPLIER_GUNS = 0.0025 // How much damage is increased (multiplier) for guns for each point of guns

// Lockpick

// Medicine
HEALTH_MULTIPLIER_MEDICINE = 0.01 // How much health is increased (multiplier) by aid items for each point of medicine

// Melee Weapons
DAMAGE_MULTIPLIER_MELEEWEAPONS = 0.01 // How much damage is increased (multiplier) for melee weapons for each point of melee weapons


// Repair

// Science
DAMAGE_MULTIPLIER_SCIENCE = 0.005 // How much damage is increased (multiplier) for plasma weapons for each point of science

// Sneak

// Speech

// Survival
NPCDAMAGE_MULTIPLIER_SURVIVAL = 0.005 // How much damage is reduced (multiplier) for npcs for each point of survival

// Unarmed
DAMAGE_MULTIPLIER_UNARMED = 0.01 // How much damage is increased (multiplier) for unarmed for each point of unarmed
